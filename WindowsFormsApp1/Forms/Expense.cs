...
namespace WindowsFormsApp1.Forms
{
    public partial class Expense : MetroFramework.Forms.MetroForm
    {
        ...
        void show_expense()
        {
            ...
            con.Close();
            String query1 = "Select Expense.id,Expense.extype,Vendor.name,Expense.exDate,Expense.totalAmount From Expense Inner join Vendor on Expense.Vendorid =Vendor.id order by Expense.id desc ";
            ...
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                ...
                metroGrid1.Rows.Add(srn, ds.Tables[0].Rows[i]["extype"].ToString(), ds.Tables[0].Rows[i]["name"].ToString(), ds.Tables[0].Rows[i]["exDate"].ToString(), ds.Tables[0].Rows[i]["totalAmount"].ToString());
                con.Close();
            }
        }
        ...
        private void savbtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Close();
                String query1 = "Select id from Vendor where name='" + costumercmb.Text + "'";
                SqlCommand sq1 = new SqlCommand(query1, con);
                con.Open();
                ...
                con.Close();
                String query = "Insert into Expense(extype, Vendorid,exDate,totalAmount) values('" + descriptiontxt.Text + "'," + costumerid + ",'" + datecmb.Text + "'," + totalamttxt.Text + ")";
                SqlCommand sq = new SqlCommand(query, con);
                con.Open();
                sq.CommandType = CommandType.Text;
                SqlDataAdapter sd = new SqlDataAdapter(sq);
        ...
        private void costumercmb_Leave(object sender, EventArgs e)
        {
            ...
            {
                con.Close();
                String query = "Select * from Vendor where name ='" + costumercmb.Text + "'";
                SqlCommand sq = new SqlCommand(query, con);
                con.Open();
                sq.CommandType = CommandType.Text;
                SqlDataAdapter sd = new SqlDataAdapter(sq);
        ...
        void show_byname()
        {
            ...
            con.Close();
            String query = "Select Expense.extype,Vendor.name,Expense.exDate,Expense.totalAmount from Expense inner join Vendor on Expense.Vendorid=Vendor.id where Vendorid=(Select id From Vendor where name='" + searchnametxt.Text + "')";
            SqlCommand sq = new SqlCommand(query, con);
            con.Open();
            SqlDataAdapter sd = new SqlDataAdapter(sq);
            DataSet ds = new DataSet();
        ...
        void show_bydate()
        {
            ...
            con.Close();
            String query = "Select Expense.extype,Vendor.name,Expense.exDate,Expense.totalAmount from Expense inner join Vendor on Expense.Vendorid=Vendor.id where exDate  between'" + datefromcmb.Text + "'and '" + datetocmb.Text + "'";
            SqlCommand sq = new SqlCommand(query, con);
            con.Open();
            SqlDataAdapter sd = new SqlDataAdapter(sq);
            DataTable ds = new DataTable();
            ...
            for (int i = 0; i < ds.Rows.Count; i++)
            {
                ...
                metroGrid1.Rows.Add(srn, ds.Rows[i]["extype"].ToString(), ds.Rows[i]["name"].ToString(), ds.Rows[i]["exDate"].ToString(), ds.Rows[i]["totalAmount"].ToString());
                con.Close();
            }
        }
        void show_bynamedate()
        {
            ...
            con.Close();
            String query = "Select Expense.extype,Vendor.name,Expense.exDate,Expense.totalAmount from Expense inner join Vendor on Expense.Vendorid=Vendor.id where exDate  between'" + datefromcmb.Text + "'and '" + datetocmb.Text + "' and Expense.Vendorid=(Select id From Vendor where name='" + searchnametxt.Text + "')";
            SqlCommand sq = new SqlCommand(query, con);
            con.Open();
            SqlDataAdapter sd = new SqlDataAdapter(sq);
            DataTable ds = new DataTable();
